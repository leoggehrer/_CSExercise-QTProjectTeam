//@CodeCopy
//MdStart
#if ACCOUNT_ON
namespace QTProjectTeam.WebApi.Models.Account
{
    /// <summary>
    /// This model represents an account user.
    /// </summary>
    public partial class IdentityUserEdit
    {
        /// <summary>
        /// Gets or sets the identity id.
        /// </summary>
        public IdType IdentityId { get; set; }
        /// <summary>
        /// Gets and sets the user first name.
        /// </summary>
        public string FirstName { get; set; } = string.Empty;
        /// <summary>
        /// Gets and sets the user last name.
        /// </summary>
        public string LastName { get; set; } = string.Empty;

        /// <summary>
        /// Creates an instance of Identity and copies the properties of the same name from the object parameter. 
        /// </summary>
        /// <param name="source">The object to copy.</param>
        /// <returns></returns>
        public static IdentityUserEdit Create(object source)
        {
            var result = new IdentityUserEdit();

            result.CopyFrom(source);
            return result;
        }
    }
}
#endif
//MdEnd
